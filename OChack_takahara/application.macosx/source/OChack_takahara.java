...
public class OChack_takahara extends PApplet {
...
public void draw() {
  ...
  float mic = map(amp.analyze(), 0, 1, 0, width);
  for (int x=0; x < width; x++) {
    float stripe = map(x, 0, width/45, 0, mic);
    float nami = sin(stripe)*1.4f;
    line(x, 0, x, width*nami);
  }

  stroke(255);
  for (int x=0; x < width; x++) {
    float stripe = map(x, 0, width/50, 0, mic);
    float nami = sin(stripe)*1.4f;
    line(x, 0, x, width*nami);
  }
}
...
public void keyPressed() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "OChack_takahara" };
    if (passedArgs != null) {
